    ...
    public interface IAreaEffect
        {
            ...
            // Determine current lighting based on day/night cycle
            // Based on nwmain.exe: Lighting calculations use _dayNightCycle and _isNight
            bool isNight = (_isNight != 0);
            float cycleProgress = _dayNightCycle / 255.0f; // Cycle is 0-255

            // Interpolate between sun and moon colors based on cycle
            ...
            if (isNight)
            {
                // Night: Use moon colors
                ambientColor = new Vector3(
                    ((_moonAmbientColor >> 16) & 0xFF) / 255.0f,
                    ((_moonAmbientColor >> 8) & 0xFF) / 255.0f,
                    (_moonAmbientColor & 0xFF) / 255.0f
                );
                diffuseColor = new Vector3(
                    ((_moonDiffuseColor >> 16) & 0xFF) / 255.0f,
                    ((_moonDiffuseColor >> 8) & 0xFF) / 255.0f,
                    (_moonDiffuseColor & 0xFF) / 255.0f
                );
                fogColor = _moonFogColor;
                fogAmount = _moonFogAmount;
            }
            else
            {
                // Day: Use sun colors
                ambientColor = new Vector3(
                    ((_sunAmbientColor >> 16) & 0xFF) / 255.0f,
                    ((_sunAmbientColor >> 8) & 0xFF) / 255.0f,
                    (_sunAmbientColor & 0xFF) / 255.0f
                );
                diffuseColor = new Vector3(
                    ((_sunDiffuseColor >> 16) & 0xFF) / 255.0f,
                    ((_sunDiffuseColor >> 8) & 0xFF) / 255.0f,
                    (_sunDiffuseColor & 0xFF) / 255.0f
                );
                fogColor = _sunFogColor;
                fogAmount = _sunFogAmount;
            }
            ...
            // Set directional light (sun/moon direction)
            // Based on nwmain.exe: Sun/moon direction calculated from cycle
            // Simplified: Use cycle to determine light direction
            float lightAngle = cycleProgress * MathHelper.TwoPi;
            Vector3 lightDirection = new Vector3(
                (float)Math.Cos(lightAngle),
                (float)Math.Sin(lightAngle) * 0.5f, // Lower angle for more realistic lighting
                (float)Math.Sin(lightAngle)
            );
            lightDirection = Vector3.Normalize(lightDirection);

            // Apply directional light properties if supported
            // Note: BasicEffect may not support all these properties - this is framework-dependent
            basicEffect.DirectionalLight0.Enabled = true;
            basicEffect.DirectionalLight0.Direction = lightDirection;
            basicEffect.DirectionalLight0.DiffuseColor = diffuseColor;

            // Apply fog if enabled and supported
            if (_fogEnabled && fogAmount > 0)
            {
                Vector3 fogColorVec = new Vector3(
                    ((fogColor >> 16) & 0xFF) / 255.0f,
                    ((fogColor >> 8) & 0xFF) / 255.0f,
                ...
                // Fog application depends on BasicEffect implementation
                // This is a simplified approach - full implementation would need fog support
            }

            // Apply shadow opacity if shadows are enabled
            // Note: Shadow rendering is complex and would require additional shadow mapping
            // This is a simplified approach for basic directional lighting
            if ((isNight && _moonShadows != 0) || (!isNight && _sunShadows != 0))
            {
                float shadowOpacity = _shadowOpacity / 255.0f;
                // Shadow application would require shadow mapping implementation
                // This is noted for future enhancement
            }
        }

        /// <summary>
        /// Renders visible tiles with proper transforms and lighting.
        /// </summary>
        /// <param name="graphicsDevice">The graphics device for rendering.</param>
        /// <param name="roomRenderer">The room mesh renderer for loading tile meshes.</param>
        /// <param name="basicEffect">The basic effect for rendering.</param>
        /// <param name="viewMatrix">The view matrix.</param>
        /// <param name="projectionMatrix">The projection matrix.</param>
        /// <param name="cameraPosition">The camera position for culling.</param>
        /// <remarks>
        /// Based on nwmain.exe: CNWSArea::RenderTiles renders visible tiles.
        /// Aurora tile rendering involves:
        /// - Loading tile meshes from MDL files referenced by tileset
        /// - Applying tile transforms (position, rotation based on orientation)
        /// - Visibility culling based on adjacency to current tile
        /// - Surface material application for walkmesh physics
        ///
        /// Tile rendering process:
        /// 1. Iterate through visible tiles in _sceneData.Tiles
        /// 2. Load or get cached tile mesh using roomRenderer
        /// 3. Apply tile transform (position + rotation based on orientation)
        /// 4. Set up effect matrices and lighting
        /// 5. Render tile mesh
        /// </remarks>
        private void RenderTiles(IGraphicsDevice graphicsDevice, IRoomMeshRenderer roomRenderer,
            IBasicEffect basicEffect, Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix, Vector3 cameraPosition)
        {
            if (_sceneData == null || _sceneData.Tiles == null || graphicsDevice == null ||
                roomRenderer == null || basicEffect == null)
            {
                return;
            }
            ...
            foreach (SceneTile tile in _sceneData.Tiles)
            {
                if (tile == null || !tile.IsVisible || string.IsNullOrEmpty(tile.ModelResRef))
                {
                    continue;
                }
                ...
                {
                    // Try to load mesh using room renderer
                    // Note: This assumes roomRenderer can load tile meshes by resref
                    // Implementation may need adjustment based on actual IRoomMeshRenderer interface
                    meshData = roomRenderer.LoadMesh(tile.ModelResRef);
                    if (meshData != null)
                    {
                        tile.MeshData = meshData; // Cache for future use
                    }
                }
                ...
                // Apply tile rotation based on orientation (0-3: 0°, 90°, 180°, 270° CCW)
                if (tile.Orientation != 0)
                {
                    float rotationAngle = tile.Orientation * MathHelper.PiOver2; // 90° increments
                    Matrix4x4 rotation = MatrixHelper.CreateRotationY(rotationAngle);
                    tileWorld = Matrix4x4.Multiply(rotation, tileWorld);
                }

                // Apply height offset based on tile height transitions
                if (tile.Height != 0)
                {
                    // Height transitions affect vertical position
                    // Simplified: Each height unit raises tile by some amount
                    const float heightScale = 1.0f; // May need adjustment based on Aurora specs
                    Vector3 heightOffset = new Vector3(0, tile.Height * heightScale, 0);
                    Matrix4x4 heightTransform = MatrixHelper.CreateTranslation(heightOffset);
                    tileWorld = Matrix4x4.Multiply(heightTransform, tileWorld);
                }

                // Set world matrix for effect
                basicEffect.World = tileWorld;

                // Apply texture if available
                // Note: Tile textures would be loaded from TPC files based on tileset
                // This is a simplified approach - full implementation would need texture loading
                ...
                graphicsDevice.DrawIndexedPrimitives(
                    ...
                    0,              // base vertex
                    0,              // start index
                    meshData.IndexCount / 3  // primitive count (triangles)
                );
            }
        }
        ...
        /// <remarks>
        /// Based on nwmain.exe: CNWSArea::RenderWeather renders weather particles.
        /// Aurora weather system includes:
        /// - Rain: Particle system with falling droplets
        /// - Snow: Particle system with falling snowflakes
        /// - Lightning: Screen flash effects with timing
        ///
        /// Weather state is updated in Update() method based on:
        /// - _chanceRain, _chanceSnow, _chanceLightning: Probability values (0-100)
        /// - _windPower: Affects particle movement
        /// - _isRaining, _isSnowing, _isLightning: Current active weather states
        ///
        /// Rendering approach:
        /// - Rain/Snow: Generate particle quads with movement based on wind and gravity
        /// - Lightning: Screen-space flash effect during lightning flash timer
        /// </remarks>
        private void RenderWeatherEffects(IGraphicsDevice graphicsDevice, IBasicEffect basicEffect,
            Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            ...
            if (_isLightning && _lightningFlashTimer > 0.0f)
            {
                // Lightning flash: Bright white screen overlay
                // Simplified: This would require a full-screen quad with additive blending
                // For now, we note the lightning flash timing is handled in Update()
                // Full implementation would render a bright white quad over the entire screen
            }

            // Render rain particles if raining
            if (_isRaining)
            {
                RenderRainParticles(graphicsDevice, basicEffect, viewMatrix, projectionMatrix);
            }

            // Render snow particles if snowing
            if (_isSnowing)
            {
                RenderSnowParticles(graphicsDevice, basicEffect, viewMatrix, projectionMatrix);
            }
        ...
        /// <summary>
        /// Renders rain particle effects.
        /// </summary>
        /// <remarks>
        /// Rain particles fall downward with wind influence.
        /// Based on nwmain.exe weather particle system.
        /// </remarks>
        private void RenderRainParticles(IGraphicsDevice graphicsDevice, IBasicEffect basicEffect,
            Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            // Simplified rain rendering
            // Full implementation would use particle system with:
            // - Rain droplet quads oriented to camera
            // - Wind-based horizontal movement
            // - Gravity-based vertical movement
            // - Distance-based fading and density
            // Note: This is a placeholder for full particle system implementation
        }

        /// <summary>
        /// Renders snow particle effects.
        /// </summary>
        /// <remarks>
        /// Snow particles fall slower than rain with wind influence.
        /// Based on nwmain.exe weather particle system.
        /// Snow particles are rendered as billboarded quads facing the camera.
        /// Particle system includes wind effects, gravity, and distance fading.
        /// </remarks>
        private void RenderSnowParticles(IGraphicsDevice graphicsDevice, IBasicEffect basicEffect,
            Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            if (graphicsDevice == null || basicEffect == null)
            {
            ...
            // Extract camera position from view matrix for distance calculations
            ...
            // Generate snow particles based on area bounds and weather intensity
            var snowParticles = GenerateSnowParticles(cameraPosition, basicEffect, viewMatrix, projectionMatrix);
            ...
            // Create vertex buffer for snow particle quads
            var vertices = CreateSnowParticleVertices(snowParticles, cameraPosition);
            ...
            var indices = CreateQuadIndices(snowParticles.Count);

            // Create vertex and index buffers
            var vertexBuffer = graphicsDevice.CreateVertexBuffer(vertices);
            var indexBuffer = graphicsDevice.CreateIndexBuffer(indices, true); // 16-bit indices

            // Configure basic effect for snow rendering
            basicEffect.World = Matrix4x4.Identity;
            basicEffect.View = viewMatrix;
            basicEffect.Projection = projectionMatrix;
            basicEffect.VertexColorEnabled = true;
            basicEffect.TextureEnabled = false; // Snow particles use vertex colors only
            basicEffect.LightingEnabled = false; // Disable lighting for particles
            basicEffect.Alpha = 0.8f; // Slight transparency for snow

            // Set blend state for alpha blending
            var blendState = graphicsDevice.CreateBlendState();
            ...
            // Set depth state (particles should be depth-tested but not write depth)
            var depthState = graphicsDevice.CreateDepthStencilState();
            depthState.DepthBufferEnable = true;
            depthState.DepthBufferWriteEnable = false; // Don't write to depth buffer
            graphicsDevice.SetDepthStencilState(depthState);

            // Set rasterizer state (no culling for billboarded particles)
            var rasterizerState = graphicsDevice.CreateRasterizerState();
            rasterizerState.CullMode = CullMode.None; // Render both sides for billboards
        ...
        /// <summary>
        /// Generates snow particles for rendering.
        /// </summary>
        /// <param name="cameraPosition">Current camera position.</param>
        /// <param name="basicEffect">Basic effect for rendering.</param>
        /// <param name="viewMatrix">View matrix.</param>
        /// <param name="projectionMatrix">Projection matrix.</param>
        /// <returns>List of snow particles to render.</returns>
        /// <remarks>
        /// Based on nwmain.exe snow particle generation algorithm.
        /// Particles are distributed in a volume around the camera with density based on weather intensity.
        /// </remarks>
        private List<SnowParticle> GenerateSnowParticles(Vector3 cameraPosition, IBasicEffect basicEffect,
            Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            var particles = new List<SnowParticle>();

            // Calculate particle density based on snow intensity (ChanceSnow)
            // Higher chance = more particles
            float densityMultiplier = Math.Max(0.1f, _chanceSnow / 100.0f);
            int baseParticleCount = (int)(50 * densityMultiplier); // Base count of 50 particles

            // Limit particle count for performance
            int maxParticles = 200;
            int particleCount = Math.Min(baseParticleCount, maxParticles);
            ...
            // Calculate visible area bounds for particle placement
            // Particles should be distributed in a volume around the camera
            float viewDistance = 50.0f; // Maximum render distance for snow
            float horizontalSpread = 30.0f; // Horizontal spread of snow volume
            float verticalSpread = 20.0f; // Vertical spread (particles fall from above)

            // Create random number generator seeded by area and time for consistent particle placement
            // This ensures particles don't jump around erratically
            int seed = GetHashCode() ^ (int)(_weatherCheckTimer * 1000);
            var random = new System.Random(seed);

            for (int i = 0; i < particleCount; i++)
            {
                // Generate particle position relative to camera
                float x = (float)(random.NextDouble() - 0.5) * horizontalSpread * 2;
                float y = (float)random.NextDouble() * verticalSpread; // Height above camera
                float z = (float)(random.NextDouble() - 0.5) * horizontalSpread * 2;

                // Add camera position offset
                Vector3 particlePos = cameraPosition + new Vector3(x, y, z);

                // Apply wind offset over time (particles drift with wind)
                float windInfluence = _windPower / 10.0f; // Scale wind power
                float timeOffset = (float)(_weatherCheckTimer % 10.0); // Cyclic time for smooth movement
                particlePos.X += windInfluence * timeOffset * 2.0f;

                // Calculate distance to camera for sorting and fading
                Vector3 toCamera = particlePos - cameraPosition;
                float distance = toCamera.Length();

                // Skip particles beyond view distance
                ...
                var particle = new SnowParticle
                {
                    ...
                    Size = 0.05f + (float)random.NextDouble() * 0.1f, // Size variation 0.05-0.15
                    Rotation = (float)random.NextDouble() * Math.PI * 2, // Random rotation
                    FallSpeed = 1.0f + (float)random.NextDouble() * 0.5f, // Fall speed variation
                    WindOffset = (float)random.NextDouble() * windInfluence, // Individual wind variation
                    Color = new Color(0.9f, 0.9f, 0.95f, 0.6f + (float)random.NextDouble() * 0.3f), // White with alpha variation
            ...
            // Sort particles by distance (back to front for proper alpha blending)
            particles.Sort((a, b) => b.DistanceToCamera.CompareTo(a.DistanceToCamera));

            return particles;
        }

        /// <summary>
        /// Creates vertex data for snow particle quads.
        /// </summary>
        /// <param name="particles">List of snow particles.</param>
        /// <param name="cameraPosition">Camera position for billboarding.</param>
        /// <returns>Array of vertices for rendering.</returns>
        /// <remarks>
        /// Each particle is rendered as a billboarded quad (two triangles).
        /// Quads are oriented to face the camera for proper 3D appearance.
        /// </remarks>
        private VertexPositionColor[] CreateSnowParticleVertices(List<SnowParticle> particles, Vector3 cameraPosition)
        {
            ...
            foreach (var particle in particles)
            {
                ...
                {
                    toCamera = Vector3.UnitZ; // Fallback if particle is at camera position
                }
                toCamera = Vector3.Normalize(toCamera);

                // Create right and up vectors perpendicular to camera direction
                Vector3 right = Vector3.Cross(toCamera, Vector3.UnitY);
                if (right.LengthSquared() < 0.001f)
                {
                    right = Vector3.Cross(toCamera, Vector3.UnitZ);
                }
                ...
                float cosRot = (float)Math.Cos(particle.Rotation);
                float sinRot = (float)Math.Sin(particle.Rotation);
                Vector3 rotatedRight = right * cosRot + up * sinRot;
                Vector3 rotatedUp = -right * sinRot + up * cosRot;

                // Calculate quad corners
                float halfSize = particle.Size * 0.5f;
                Vector3 topLeft = particle.Position - rotatedRight * halfSize + rotatedUp * halfSize;
                Vector3 topRight = particle.Position + rotatedRight * halfSize + rotatedUp * halfSize;
                Vector3 bottomLeft = particle.Position - rotatedRight * halfSize - rotatedUp * halfSize;
                Vector3 bottomRight = particle.Position + rotatedRight * halfSize - rotatedUp * halfSize;

                // Apply distance-based fading
                float fadeFactor = 1.0f - Math.Min(1.0f, particle.DistanceToCamera / 40.0f);
                Color fadedColor = new Color(
                    particle.Color.R,
                    particle.Color.G,
                    particle.Color.B,
                    (byte)(particle.Color.A * fadeFactor)
                );

                // Add quad vertices (counter-clockwise winding)
                vertices.Add(new VertexPositionColor(bottomLeft, fadedColor));  // 0
                vertices.Add(new VertexPositionColor(topLeft, fadedColor));     // 1
                vertices.Add(new VertexPositionColor(topRight, fadedColor));    // 2
                vertices.Add(new VertexPositionColor(bottomRight, fadedColor)); // 3
            }
        ...
        /// <summary>
        /// Creates index buffer for quad rendering.
        /// </summary>
        /// <param name="particleCount">Number of particles.</param>
        /// <returns>Array of indices for quad triangles.</returns>
        /// <remarks>
        /// Each quad consists of 2 triangles (6 indices).
        /// Indices reference vertices in the order: bottom-left, top-left, top-right, bottom-right.
        /// </remarks>
        private int[] CreateQuadIndices(int particleCount)
        {
            var indices = new int[particleCount * 6]; // 6 indices per quad (2 triangles)

            for (int i = 0; i < particleCount; i++)
            {
                int vertexOffset = i * 4; // 4 vertices per quad
                int indexOffset = i * 6;  // 6 indices per quad
        ...
        /// <remarks>
        /// Based on nwmain.exe: CNWSArea::RenderAreaEffects renders persistent effects.
        /// Area effects are stored in _areaEffects list and implement IAreaEffect interface.
        /// Each effect handles its own rendering through the Render() method.
        ///
        /// Rendering process:
        /// 1. Iterate through all active area effects
        /// 2. Call Render() on each effect
        /// 3. Effects handle their own graphics setup and drawing
        /// </remarks>
        private void RenderAreaEffects(IGraphicsDevice graphicsDevice, IBasicEffect basicEffect,
            Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            if (_areaEffects == null || _areaEffects.Count == 0)
            {
                return;
            }

            // Render each active area effect
            // Based on nwmain.exe: Area effects are rendered as part of area rendering pipeline
            foreach (IAreaEffect effect in _areaEffects)
            {
                if (effect != null && effect.IsActive)
                {
                    // Each effect handles its own rendering
                    // Effects may use the provided graphics device and matrices
                    effect.Render();
                }
            }
        }
