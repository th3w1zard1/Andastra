...
namespace BioWare.NET.Common
{
    ...
    public abstract class BinaryFormatReaderBase
    {
        ...
        protected readonly BioWare.NET.Common.RawBinaryReader Reader;

        protected BinaryFormatReaderBase(byte[] data, [CanBeNull] Encoding encoding = null)
        {
            Data = data;
            Reader = BioWare.NET.Common.RawBinaryReader.FromBytes(data, 0, null);
        }
        ...
        {
            Data = data;
            Reader = BioWare.NET.Common.RawBinaryReader.FromBytes(data, offset, size);
        }
        ...
        protected BinaryFormatReaderBase(Stream source, int offset, int? size, [CanBeNull] Encoding encoding = null)
        {
            if (source.CanSeek)
            {
                Reader = BioWare.NET.Common.RawBinaryReader.FromStream(source, offset, size);
