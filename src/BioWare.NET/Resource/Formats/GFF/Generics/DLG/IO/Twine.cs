...
namespace BioWare.NET.Resource.Formats.GFF.Generics.DLG.IO
{
    /// <summary>
    ...
    public static class Twine
    {
        ...
        private static TwineStory ReadJson(string content)
        {
            ...
            var twineMetadata = new TwineMetadata
            {
                ...
                Creator = root.TryGetProperty("creator", out var creatorProp) ? creatorProp.GetString() ?? "BioWare.NET" : "BioWare.NET",
        ...
        private static TwineStory ReadHtml(string content)
        {
            ...
            var twineMetadata = new TwineMetadata
            {
                ...
                Creator = storyData.Attribute("creator")?.Value ?? "BioWare.NET",
        ...
        private static DLG StoryToDlg(TwineStory story)
        {
            ...
            foreach (var passage in story.Passages)
            {
                ...
                node.Text = new BioWare.NET.Common.LocalizedString(-1);
                node.Text.SetData(BioWare.NET.Common.Language.English, BioWare.NET.Common.Gender.Male, passage.Text);
                        ...
                        if (parts[0] == "text")
                        {
                            try
                            {
                                ...
                                if (int.TryParse(parts[2], out int genderVal))
                                {
                                    ...
                                    if (Enum.TryParse<BioWare.NET.Common.Language>(langName, true, out BioWare.NET.Common.Language lang))
                                    {
                                        node.Text.SetData(lang, (BioWare.NET.Common.Gender)genderVal, value);
                                    }
                                }
                            }
                            catch
        ...
        private static TwineStory DlgToStory(DLG dlg, Dictionary<string, object> metadata = null)
        {
            ...
            var storyMeta = new TwineMetadata
            {
                ...
                Zoom = zoomVal,
                Creator = meta.ContainsKey("creator") ? meta["creator"].ToString() : "BioWare.NET",
            ...
            TwinePassage EnsurePassage(DLGNode node, string pid)
            {
                ...
                string primaryText = node.Text?.GetString(BioWare.NET.Common.Language.English, BioWare.NET.Common.Gender.Male) ?? "";
