...
namespace BioWare.NET.Resource.Formats.GFF.Generics.DLG
{
    ...
    public static class DLGHelper
    {
        ...
        private static void ConstructNode(GFFStruct gffStruct, DLGNode node)
        {
            ...
            node.Sound = ConvertResRefFromParsing(gffStruct.Acquire("Sound", BioWare.NET.Common.ResRef.FromBlank()));
            ...
            node.Script2 = ConvertResRefFromParsing(gffStruct.Acquire("Script2", BioWare.NET.Common.ResRef.FromBlank()));
        ...
        private static BioWare.NET.Common.ResRef ConvertResRefFromParsing(BioWare.NET.Common.ResRef parsingResRef)
        {
            ...
            return new BioWare.NET.Common.ResRef(parsingResRef.ToString());
        }

        private static BioWare.NET.Common.LocalizedString ConvertLocalizedString(BioWare.NET.Common.LocalizedString coreLocalizedString)
        {
            if (coreLocalizedString == null)
            {
                return BioWare.NET.Common.LocalizedString.FromInvalid();
            }
            var result = new BioWare.NET.Common.LocalizedString(coreLocalizedString.StringRef);
            // Copy all language/gender combinations
            foreach (BioWare.NET.Common.Language lang in System.Enum.GetValues(typeof(BioWare.NET.Common.Language)))
            {
                foreach (BioWare.NET.Common.Gender gender in System.Enum.GetValues(typeof(BioWare.NET.Common.Gender)))
                {
                    string text = coreLocalizedString.Get(lang, gender);
                    if (!string.IsNullOrEmpty(text))
                    {
                        result.SetData((BioWare.NET.Common.Language)(int)lang, (BioWare.NET.Common.Gender)(int)gender, text);
                    }
                }
            }
            return result;
        ...
        private static BioWare.NET.Common.Color ConvertColorToParsing(System.Drawing.Color drawingColor)
        {
        ...
        private static BioWare.NET.Common.ResRef ConvertResRefToParsing(BioWare.NET.Common.ResRef coreResRef)
        {
            if (coreResRef == null)
            {
                return BioWare.NET.Common.ResRef.FromBlank();
        ...
        private static BioWare.NET.Common.LocalizedString ConvertLocalizedStringToParsing(BioWare.NET.Common.LocalizedString coreLocalizedString)
        {
            if (coreLocalizedString == null)
            {
                return BioWare.NET.Common.LocalizedString.FromInvalid();
