...
namespace BioWare.NET.Resource.Formats.GFF.Generics.CNV
{
    ...
    public static class CNVHelper
    {
        ...
        public static CNV ConstructCnv(GFF gff)
        {
            ...
            cnv.OnAbort = root.Acquire("EndConverAbort", BioWare.NET.Common.ResRef.FromBlank());
            cnv.OnEnd = root.Acquire("EndConversation", BioWare.NET.Common.ResRef.FromBlank());
            cnv.Skippable = root.Acquire("Skippable", (byte)0) != 0;
            cnv.AmbientTrack = root.Acquire("AmbientTrack", BioWare.NET.Common.ResRef.FromBlank());
            cnv.AnimatedCut = root.Acquire("AnimatedCut", 0);
            cnv.CameraModel = root.Acquire("CameraModel", BioWare.NET.Common.ResRef.FromBlank());
            ...
            foreach (GFFStruct stuntStruct in stuntList)
            {
                ...
                stunt.StuntModel = stuntStruct.Acquire("StuntModel", BioWare.NET.Common.ResRef.FromBlank());
        ...
        {
            node.Text = gffStruct.Acquire("Text", BioWare.NET.Common.LocalizedString.FromInvalid());
            node.Listener = gffStruct.Acquire("Listener", string.Empty);
            node.VoResRef = gffStruct.Acquire("VO_ResRef", BioWare.NET.Common.ResRef.FromBlank());
            node.Script1 = gffStruct.Acquire("Script", BioWare.NET.Common.ResRef.FromBlank());
            ...
            node.Sound = gffStruct.Acquire("Sound", BioWare.NET.Common.ResRef.FromBlank());
            ...
            node.Script2 = gffStruct.Acquire("Script2", BioWare.NET.Common.ResRef.FromBlank());
        ...
        {
            link.Active1 = gffStruct.Acquire("Active", BioWare.NET.Common.ResRef.FromBlank());
            link.Active2 = gffStruct.Acquire("Active2", BioWare.NET.Common.ResRef.FromBlank());
        ...
        /// </remarks>
        public static GFF DismantleCnv(CNV cnv, BioWare.NET.Common.BioWareGame game)
        {
        ...
        private static void DismantleNode(GFFStruct gffStruct, CNVNode node, List<CNVEntry> allEntries, List<CNVReply> allReplies, string listName, BioWare.NET.Common.BioWareGame game)
        {
        ...
        private static void DismantleLink(GFFStruct gffStruct, CNVLink link, BioWare.NET.Common.BioWareGame game, string listName)
        {
        ...
        public static byte[] BytesCnv(CNV cnv, BioWare.NET.Common.BioWareGame game, ResourceType fileFormat = null)
        {
            if (fileFormat == null)
            {
        ...
        // Helper methods to convert between BioWare.NET.Common and BioWare.NET.Common types
        private static System.Drawing.Color ConvertColor(BioWare.NET.Common.Color color)
        {
            if (color == null)
            {
        ...
        private static BioWare.NET.Common.ResRef ConvertResRef(BioWare.NET.Common.ResRef resRef)
        {
            if (resRef == null)
            {
                return BioWare.NET.Common.ResRef.FromBlank();
            }
            return new BioWare.NET.Common.ResRef(resRef.ToString());
        }

        private static BioWare.NET.Common.LocalizedString ConvertLocalizedString(BioWare.NET.Common.LocalizedString localizedString)
        {
            if (localizedString == null)
            {
                return BioWare.NET.Common.LocalizedString.FromInvalid();
            }
            var result = new BioWare.NET.Common.LocalizedString(localizedString.StringRef);
            // Copy all language/gender combinations
            foreach (var lang in System.Enum.GetValues(typeof(BioWare.NET.Common.Language)))
            {
                foreach (var gender in System.Enum.GetValues(typeof(BioWare.NET.Common.Gender)))
                {
                    string text = localizedString.Get((BioWare.NET.Common.Language)lang, (BioWare.NET.Common.Gender)gender);
                    if (!string.IsNullOrEmpty(text))
                    {
                        // Convert Language and Gender enums
                        BioWare.NET.Common.Language coreLang = (BioWare.NET.Common.Language)(int)lang;
                        BioWare.NET.Common.Gender coreGender = (BioWare.NET.Common.Gender)(int)gender;
