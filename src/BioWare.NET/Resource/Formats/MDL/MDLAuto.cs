...
namespace BioWare.NET.Resource.Formats.MDL
{
    ...
    public static class MDLAuto
    {
        ...
        private static BioWare.NET.Common.RawBinaryReader CreateReader(object source, int offset, int? size = null)
        {
            if (source is string path)
            {
                return BioWare.NET.Common.RawBinaryReader.FromFile(path, offset, size);
            }
            if (source is byte[] bytes)
            {
                return BioWare.NET.Common.RawBinaryReader.FromBytes(bytes, offset, size);
            }
            if (source is Stream stream)
            {
                return BioWare.NET.Common.RawBinaryReader.FromStream(stream, offset, size);
            }
